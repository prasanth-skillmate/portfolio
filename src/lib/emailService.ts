...
export const sendEmailViaAPI = async (formData: ContactFormData): Promise<boolean> => {
  try {
    ...
    const API_BASE_URL = import.meta.env.VITE_API_BASE_URL || 
      (isLocalhost ? 'http://localhost:3001' : `http://${host}:3001`);
    ...
    if (error instanceof TypeError && error.message.includes('fetch')) {
      throw new Error('Unable to connect to email server. Please ensure the backend server is running on port 3001.');
    }
    throw error;
  }
};
  ...
  try {
    ...
    if (EMAILJS_SERVICE_ID && EMAILJS_TEMPLATE_ID && EMAILJS_PUBLIC_KEY) {
      try {
        ...
        throw new Error('Failed to send email. Please try again later or contact directly at cybersecure8@gmail.com');
      }
    }
